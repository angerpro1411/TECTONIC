...
static inline ap_uint<16> uf_find(ap_uint<16> parent[512], ap_uint<16> x) {

#pragma HLS INLINE
 ap_uint<16> root = x;
 VITIS_LOOP_26_1: while(parent[root] != root) root = parent[root];
 ap_uint<16> cur = x;
 VITIS_LOOP_28_2: while(parent[cur] != root) {
...
static inline void uf_union(ap_uint<16> parent[512], ap_uint<16> a, ap_uint<16> b) {
...
__attribute__((sdx_kernel("hls_object_green_classification", 0))) void hls_object_green_classification(
 hls::stream<axis_rgb_t> &in_stream,
 hls::stream<axis_rgb_t> &out_stream,
 int16_t obj_x[32],
 int16_t obj_y[32],
 ap_uint<1> obj_is_green[32],
 ap_uint<16>& object_count) {
#line 19 "/home/dell3561-49/Vitis_HLS_folder/Obj_detect_ver6/Obj_Detect_ver6/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=hls_object_green_classification
# 51 "obj_detect.cpp"

#line 7 "/home/dell3561-49/Vitis_HLS_folder/Obj_detect_ver6/Obj_Detect_ver6/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=hls_object_green_classification
# 51 "obj_detect.cpp"
...
#pragma HLS INTERFACE s_axilite port=return bundle=CTRL
 ...
 static int16_t obj_x_buf[32];
 static int16_t obj_y_buf[32];
 static ap_uint<1> obj_is_green_buf[32];
 static ap_uint<16> object_count_buf;

 static ap_uint<8> imgR[240][320];
 static ap_uint<8> imgG[240][320];
 static ap_uint<8> imgB[240][320];
...
#pragma HLS BIND_STORAGE variable=imgB type=ram_2p impl=bram
 ...
 static ap_uint<1> edge_mask[240][320];

#pragma HLS BIND_STORAGE variable=edge_mask type=ram_2p impl=bram

 static ap_uint<16> label_map[240][320];

#pragma HLS BIND_STORAGE variable=label_map type=ram_2p impl=bram

 static ap_uint<16> parent[512];

#pragma HLS BIND_STORAGE variable=parent type=ram_2p impl=bram


 static ap_uint<16> min_x[512];
 static ap_uint<16> max_x[512];
 static ap_uint<16> min_y[512];
 static ap_uint<16> max_y[512];
 static ap_uint<1> center_is_green[512];
...
#pragma HLS BIND_STORAGE variable=max_y type=ram_2p impl=bram
 ...
 static ap_uint<16> pixel_count[512];
#pragma HLS BIND_STORAGE variable=pixel_count type=ram_2p impl=lutram


 InitLoop: for(int i=0;i<512;i++){

#pragma HLS PIPELINE II=1
 parent[i]=i;
 min_x[i]=320; max_x[i]=0;
 min_y[i]=240; max_y[i]=0;
 center_is_green[i]=0;
 pixel_count[i]=0;
 }


 static ap_uint<8> linebuf[2][320];
...
#pragma HLS ARRAY_PARTITION variable=window complete dim=0
 ...
 PASS_1: for(int i=0;i<240*320;i++){
...
#pragma HLS UNROLL
 ...
 linebuf[row&1][col]=gray_pix;
    ...
    col++;
    if(col==320){col=0; row++;}
}



 PASS1_5_Outer: for(int y=1; y<240 -1; y++) {
     Pass1_5_inner0: for(int x=1; x<320 -1; x++) {
#pragma HLS PIPELINE II=1
 ap_uint<1> dilated = 0;
...
#pragma HLS UNROLL
 ...
 VITIS_LOOP_206_1: for (int y=0; y<240; y++) {
     VITIS_LOOP_207_2: for (int x=0; x<320; x++) {
#pragma HLS PIPELINE II=3
 ...
 VITIS_LOOP_218_3: for (int y=0; y<240; y++) {
     VITIS_LOOP_219_4: for (int x=0; x<320; x++) {
#pragma HLS PIPELINE II=3
 ap_uint<16> lbl = label_map[y][x];
         if (lbl != 0) {
 ...
 static ap_uint<1> is_external[512];
...
#pragma HLS PIPELINE II=1
 ...
 ap_uint<16> count = 0;
 pass_4: for (ap_uint<16> i = 1; i < next_label && count < 32; i++) {
#pragma HLS PIPELINE II=3
 if(parent[i]==i &&
       is_external[i] &&
     pixel_count[i] >= 200) {
         ...
         obj_x[count] = (int16_t)(cx - 320 / 2);
         obj_y[count] = (int16_t)(240 / 2 - cy);
 ...
 VITIS_LOOP_276_5: for(int k=0; k<count; k++) {
#pragma HLS PIPELINE II=1
 obj_x_buf[k] = obj_x[k];
     obj_y_buf[k] = obj_y[k];
 ...
 pass5_out: for(int y=0;y<240;y++){
  pass5_in: for(int x=0;x<320;x++){
...
#pragma HLS PIPELINE II=1
 ...
 ap_uint<8> B=imgB[y][x];
   ...
   ap_uint<16> lbl=label_map[y][x];
   if(edge_mask[y][x] && lbl!=0 && pixel_count[lbl] >= 200)
    {
   ...
   out_pix.strb = -1;
   out_pix.user = 0;
   out_pix.last = (y==240 -1 && x==320 -1);

   out_stream.write(out_pix);
  }
 }

 VITIS_LOOP_311_6: for(int k=0; k<object_count_buf; k++) {
#pragma HLS PIPELINE II=1
 obj_x[k] = obj_x_buf[k];
     obj_y[k] = obj_y_buf[k];
