...
__attribute__((sdx_kernel("BackGrRemovalStream", 0))) void BackGrRemovalStream(hls::stream<AXI_VAL> &input_stream,
...
#pragma HLS ARRAY_PARTITION variable=h cyclic factor=32 dim=1
 ...
 static PixelBGR outBGR[(320*240)];
#pragma HLS RESOURCE variable=outBGR core=RAM_T2P_BRAM


 VITIS_LOOP_37_1: for (u32 i = 0; i < (320*240); i++) {
...
#pragma HLS UNROLL factor=4
 u32 idx = (x + i - Half_size_kernel)
                ...
                if (count >= EightyPercentOfKernel) {
                    whiten_kernel:
                    for (u32 j = 0; j < kernel_size; j++) {
                        VITIS_LOOP_75_2: for (u32 i = 0; i < kernel_size; i++) {
#pragma HLS UNROLL factor=4
 u32 idx = (x + i - Half_size_kernel)
                                    + (y + j - Half_size_kernel) * 320;
                            outBGR[idx].r = 255;
    ...
    VITIS_LOOP_89_3: for (u32 i = 0; i < (320*240); i++) {
#pragma HLS PIPELINE II=1
 inBGR[i] = outBGR[i];
    }
}
...
__attribute__((sdx_kernel("BackGrRemovalStream", 0))) void BackGrRemovalStream(hls::stream<AXI_VAL> &input_stream,
                         ...
                         u8 threshold)
{
#line 20 "/home/dell3561-49/Vitis_HLS_folder/BackGrRemoval_AXIS_ver2/BaGrRemoval_AXIs_ver2/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=BackGrRemovalStream
# 99 "BackGrRemoval.cpp"
