...
static inline void pack_axi_pixel(AXI_VAL &axi, u8 r, u8 g, u8 b, bool last=false) {
    ap_uint<24> d = (ap_uint<24>)((ap_uint<8>)r, (ap_uint<8>)g, (ap_uint<8>)b);
    axi.data = d;
    axi.keep = -1;
    axi.strb = -1;
    axi.user = 0;
...
__attribute__((sdx_kernel("BackGrRemoval_stream_v2", 0))) void BackGrRemoval_stream_v2(axis_t &in_stream, axis_t &out_stream, u8 threshold, int rows, int cols) {
#line 17 "/home/dell3561-49/Vitis_HLS_folder/BackGrRemoval_AXIS/AxiS_BackGrRemoval/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=BackGrRemoval_stream_v2
# 54 "BackGrRemoval.cpp"
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
 ...
 const int K = 11;
    ...
    static u8 linebuf[K-1][320];
#pragma HLS RESOURCE variable=linebuf core=RAM_1P_BRAM
...
#pragma HLS ARRAY_PARTITION variable=row_shift complete dim=0
 ...
 int total = rows * cols;
    ...
    VITIS_LOOP_84_1: for (int pix = 0; pix < total; pix++) {
#pragma HLS PIPELINE II=1
 AXI_VAL in_axi = in_stream.read();
        u8 r,g,b;
        unpack_axi_pixel(in_axi, r, g, b);


        u8 Hv = rgb_to_h_scaled(r,g,b);


    shift_rows:
        for (int rr = 0; rr < K; rr++) {
#pragma HLS UNROLL factor=2
 VITIS_LOOP_97_2: for (int cc = 0; cc < K-1; cc++) {
#pragma HLS UNROLL factor=2
 row_shift[rr][cc] = row_shift[rr][cc+1];
            }
        }
...
#pragma HLS UNROLL factor=2
 ...
 row_shift[rr][K-1] = linebuf[rr][col];
        }
        row_shift[K-1][K-1] = Hv;


    update_linebuf:
        for (int s = 0; s < K-2; s++) {
#pragma HLS UNROLL factor=2
 linebuf[s][col] = linebuf[s+1][col];
        }
        ...
        bool valid_center = (row >= HALF) && (row < (rows - HALF)) && (col >= HALF) && (col < (cols - HALF));

        u8 out_r = r, out_g = g, out_b = b;
...
#pragma HLS UNROLL factor=2
 VITIS_LOOP_136_3: for (int wx = 0; wx < K; wx++) {
#pragma HLS UNROLL factor=2
