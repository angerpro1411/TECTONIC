...
//// HLS-friendly Union-Find (bounded loops)
//static inline ap_uint<16> uf_find(ap_uint<16> parent[MAX_LABELS], ap_uint<16> x) {
//#pragma HLS INLINE
//    ap_uint<16> root = x;
//    // bounded climb to root
//    FIND_LOOP: for (int k = 0; k < MAX_LABELS; k++) {
//        if (parent[root] == root) break;
//        root = parent[root];
//    }
//
//    // path compression (bounded)
//    ap_uint<16> cur = x;
//    COMPRESS_LOOP: for (int k = 0; k < MAX_LABELS; k++) {
//        if (parent[cur] == root) break;
//        ap_uint<16> next = parent[cur];
//        parent[cur] = root;
//        cur = next;
//    }
