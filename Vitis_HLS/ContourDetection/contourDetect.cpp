...
#pragma HLS PIPELINE II=1
        ...
        // Extract RGB
        ap_uint<8> R = (in_pix.data >> 16) & 0xFF;
        ap_uint<8> G = (in_pix.data >> 8) & 0xFF;
        ap_uint<8> B = (in_pix.data) & 0xFF;
...
#pragma HLS UNROLL
        ...
        if (row >= 2 && col >= 2) {
            ...
            float mag_f = hls::sqrtf((float)(gx*gx + gy*gy));
            int mag = (int)(mag_f > 255 ? 255 : mag_f);

            // If edge detected, paint pixel bright green
            if (mag > threshold) {
                outR = 0;
                outG = 255;
                outB = 0;
        ...
        axis_rgb_t out_pix;
        out_pix.data = ((ap_uint<24>)outR << 16) | ((ap_uint<24>)outG << 8) | outB;
