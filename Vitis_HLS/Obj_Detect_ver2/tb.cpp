...
int main() {
    ...
    int16_t obj_y[MAX_OBJECTS];
    ap_uint<1> obj_is_green[MAX_OBJECTS];
    ap_uint<16> object_count = 0;

    if (!read_bmp(filename_input, input_image)) return 1;
    ...
    // Call DUT
    hls_object_green_classification(in_stream, out_stream,obj_x,obj_y,obj_is_green,object_count);
    ...
    write_bmp(filename_output, output_image);
    std::cout << "✅ Output image written: output_320x240.bmp" << std::endl;
    // Print detected object info
    std::cout << "Detected objects: " << object_count << std::endl;
    for (int i = 0; i < object_count; i++)
    { std::cout << "Object " << i << ": center(" << obj_x[i] << ", "
    	<< obj_y[i] << "), color = " << (obj_is_green[i] ? "GREEN" : "RED")
		<< std::endl; }
