...
def reduce_shadow(image,threshold_darkpixel=65,kernel_size=11,):
    ...
    Eighty_percent_kernel = int(total_pixel_in_kernel*0.5)
    dark_pixel_in_total = 0
    ...
    for y in range(0+half_size_kernel,height-half_size_kernel+1,1):
        for x in range(0+half_size_kernel,width-half_size_kernel+1,1):
            ...
            cnt_column = 0
            #find a dark pixel
            if v_channel[y,x] < threshold_darkpixel :
                ...
                for j in range(y-half_size_kernel,y+half_size_kernel,1):
                    for i in range(x-half_size_kernel,x+half_size_kernel,1):
                        if v_channel[j,i] < threshold_darkpixel :
                            dark_pixel_in_total = dark_pixel_in_total + 1
                        else :
                            v_copy = v_channel[j,i]
                
                if dark_pixel_in_total > Eighty_percent_kernel :
                    ...
                    for j in range(y-half_size_kernel,y+half_size_kernel):
                        ...
                        for i in range(x-half_size_kernel,x+half_size_kernel):
                            if v_channel[j,i]*1.2 < 255 :
                                v_channel[j,i] = v_channel[cnt_column,cnt_row]
                                h[j,i] = h[cnt_column,cnt_row]
                                s[j,i] = s[cnt_column,cnt_row]
                            cnt_row=cnt_row+1
            dark_pixel_in_total = 0

    imgHsv = cv2.merge([h,s,v_channel])
    new_img = cv2.cvtColor(imgHsv,cv2.COLOR_HSV2BGR)
...
image = cv2.imread("/home/dell3561-49/PYTHON_CODE/SingleTree/singletree6.png")
# image = cv2.imread("/home/dell3561-49/PYTHON_CODE/1.COLOR_CORRECTION/Images/smallmap1.png")
...
while True :
    ...
    for c in cnts:
        ...
        if (area > areaMin): # and area < areaMax :
            ...
            color = pixel_color('/home/dell3561-49/PYTHON_CODE/SingleTree/singletree6.png',cX,cY)
            # color = pixel_color('/home/dell3561-49/PYTHON_CODE/1.COLOR_CORRECTION/Images/smallmap1.png',cX,cY)
    ...
    imgStack = stackImages(0.5,([image,gray,filtered],[edges1,closing,imgContour]))
